...
* **Owner:** Uses `Ownable2Step` for secure transfers. Has strictly limited power. Can only call `rescueTokens()` for accidental ERC20s (excluding USDC/SAFWA). Cannot upgrade contract, cannot mint tokens, cannot pause, cannot access reserves.
* **Tax Wallet:** Immutable address that receives the 0.1% sell tax (capped at 100 USDC).
...
### 1. Buying SAFWA
 ...
 Buying mints new tokens. You must approve the SAFWA contract to spend your USDC first. The function is protected by `nonReentrant`.
...
### 2. Selling SAFWA
 ...
 Selling burns tokens and returns USDC to your wallet. Protected by `nonReentrant`.
...
### 3. Reading Price data
 ...
 ```solidity
 // Get USDC cost for X tokens (Rounds UP)
 uint256 cost = safwa.getBuyCost(amount);
 
 // Get USDC refund for X tokens (Rounds DOWN)
...
* **Access Control:** Implements `Ownable2Step` to prevent accidental ownership loss during transfers.
* **ReentrancyGuard:** All state-changing external functions (`buy`, `sell`) are protected against reentrancy attacks.
* **Immutable State:** The `usdc` and `taxWallet` addresses are immutable, preventing admin manipulation after deployment.
* **Rounding Strategy:**
  * Buys round **UP** (User pays slightly more).
  * Sells round **DOWN** (User gets slightly less).
  * *Result:* The contract accumulates a "mathematical surplus" (Dust) over time, ensuring it is always over-collateralized.
* **Asset Protection:** The `rescueTokens` function explicity reverts if the owner attempts to withdraw `USDC` (Reserve) or `SAFWA` (Token Supply), complying with the "Code is Law" standard.
