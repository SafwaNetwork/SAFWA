...
The **SAFWA Protocol** represents a paradigm shift in decentralized value creation. It abandons the traditional "Order Book" and "Constant Product AMM" (e.g., Uniswap ) models in favor of a **Continuous Token Bonding Curve (CTBC)**.

By mathematically coupling the circulating supply of the SAFWA token () to a reserved collateral pool of USDC (), the protocol creates a deterministic, autonomous financial market. The system guarantees **instant liquidity** at a mathematically predictable price, regardless of market conditions, volume, or external exchange solvency.
...
The Spot Price  (in USDC) for the -th token is a function of the current Total Supply .

**Parameters:**

* (Slope/Scaling Factor): **0.005**
* (Initial Price Offset): **1.00 USDC**
* : Circulating Supply (in units of  Wei).

### 3.2 The Reserve Function

The Total Reserve () held in the contract vault is the definite integral of the Price Function from  to current supply . This represents the "Market Cap" effectively backed by collateral.

Solving the integral:

This equation () is the **Solvency Invariant**. As long as the actual USDC balance in the contract is , the system is solvent.

### 3.3 Transaction Calculus

#### 3.3.1 Minting (Buy Calculation)

When a user deposits  (USDC) to mint  (tokens), we must solve for .
...
The New Reserve .
The New Supply  is derived by inverting the Reserve Function. However, since inverting a polynomial of degree  is computationally expensive in Solidity, the protocol iterates based on the user's input:
...
When a user burns  tokens to receive a USDC refund:
...
* As Supply () approaches ,  approaches .
...
3. ...
4. Last User sells: Price returns to . The Last User gets paid.
...
* **No Iterative Loops:** The integral calculation is analytical (closed-form), not numerical. This ensures that buying 1 token costs the same Gas as buying 1,000,000 tokens ( complexity).
...
**Risk:** If the Reserve Balance exceeds , calculations revert.
